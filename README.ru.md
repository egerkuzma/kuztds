...
**Эксплуатация:**
- Конфиг групп, IP-списки, списки separation и сигнатуры ботов перечитываются в фоне при изменении
  файлов — правки из админки вступают в силу без рестарта движка.
- Битый или пропавший конфиг пропускается и пробуется снова, работающие правила
  остаются на месте.
...
- Rate-limit логина (Redis, фиксированное окно на IP).
...
**Группы** — группа показана как «водопад» потоков в порядке перебора сверху
вниз. Каждая строка читается как правило: переключатель, условия (чипы), выход,
что получают боты, и хиты / боты / конверсии за период. Строку можно перетащить,
клик открывает редактор; последняя строка — что происходит, когда ничего не
совпало:

![Группы](docs/img/groups.png)

**Редактор потока** — слева «КОГДА» (только те условия, которые поток реально
использует; остальные — через «Add condition»), справа «КУДА» (выход, боты),
редкое свёрнуто в Advanced:

![Редактор потока](docs/img/stream.png)

**Настройки группы** раскрываются над водопадом по кнопке — ID и алиасы,
выход по умолчанию, уникальность, сбор ключей, антифлуд, живые ссылки движка и
опасная зона:

![Настройки группы](docs/img/group-form.png)

**Тёмная тема** — кнопка солнце/луна в топбаре; выбор запоминается в браузере,
а пока он не сделан, панель следует системной настройке:

![Дашборд, тёмная тема](docs/img/dashboard-dark.png)
...
HTTP-запрос
  ...
  ├─ separation · загрузка [REMOTE] · chance · распределение (|||) · api_mac
...
По `SIGTERM` движок останавливается в две последовательные фазы, по десять
секунд каждая: `http.Server.Shutdown` для запросов в полёте, затем буфер логов
для событий, ещё не доехавших до ClickHouse. **Дайте процессу не меньше 25
секунд на остановку.**

Бюджеты разделены намеренно: с одним общим дедлайном загруженный сервер мог
израсходовать его целиком и передать сливу логов уже истёкший контекст.

Умолчания почти везде короче — Docker убивает контейнер через 10 секунд после
`SIGTERM`, а `TimeoutStopSec` в systemd обычно меньше, чем кажется. `SIGKILL`
посреди слива стоит последнего батча событий *и* строки `engine stopped`,
которая сообщает, что именно потеряно.
...
Один встроенный SPA (тема в стиле GitHub, светлая и тёмная, английский UI).
Раскладка: **сайдбар слева**, **справа сверху** выбор периода, переключатель
темы, шестерёнка настроек, пользователь и выход.

- **Dashboard** — шесть плиток KPI (визиты, уникальные, боты, конверсии,
  профит, CR), график по времени со значениями при наведении, **таблица
  результативности** по группам и потокам (хиты / уникальные / боты /
  конверсии / профит / CR, сортировка, экспорт CSV, строка «no match → group
  default» у каждой группы) и разбивки по странам, устройствам, ОС, браузерам
  и источникам.
...
- **Groups** — слева сворачиваемое дерево группа→поток с поиском (и хитами
  за период у каждой группы и потока), справа панель, в которой либо
  **водопад** группы — по строке на поток в порядке перебора: переключатель,
  чипы условий, выход, что получают боты, хиты / боты / конверсии; перетаскивание
  для смены порядка, клик — в редактор; строка «no match» для выхода группы по
  умолчанию; настройки группы раскрываются над ним — либо редактор одного
  потока: **When** (только настроенные условия, остальные через «Add
  condition»; 19 видов: гео, устройство, бренд, WAP-оператор, ОС, браузер,
  Яндекс.Браузер, текст UA / referer / домена / ключа / языка, IP-список,
  уникальность, наличие referer, расписание, лимит показов) и **Then** (тип
  редиректа, распределение, out), **Bots** (чипы сигналов, что им отдавать) и
  свёрнутый **Advanced** (шанс показа, separation, `[REMOTE]`, CURL
  find/replace, API mac). Обе панели скроллятся внутри себя, поэтому выбор
  потока не двигает страницу. Метка несохранённых изменений, `Ctrl`+`S`,
  подтверждение при уходе и закрытии.
- **Lists** — редактор `.dat`-файлов (IP-базы, WAP-операторы, сигнатуры).
- **Settings** (шестерёнка) — смена пароля админки, переключение темы.
