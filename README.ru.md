...
**Группы и потоки** — редактор мастер-детейл: слева сворачиваемое дерево групп,
справа одна панель формы, живые ссылки, которые отдаёт движок:
...
HTTP-запрос
  ...
  ├─ группа по id/алиасу (путь запроса целиком); нет → режим «trash»
...
Обычно группы **редактируются в админке** («Группы» → выбрать группу или поток → «Save all»);
