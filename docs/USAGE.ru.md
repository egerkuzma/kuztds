...
периода, шестерёнка «Настройки», чип пользователя и кнопка выхода). Светлая тема в стиле GitHub (Primer).
...
мастер-детейл: дерево групп→потоки и одна панель формы), **Списки** (редактор
`.dat`, в т.ч. WAP-операторы). «Настройки» (смена пароля) — по шестерёнке справа.

Группы — редактор в режиме **мастер-детейл**. Слева сворачиваемое дерево
группа→поток (шеврон у группы) и поиск по именам групп и потоков. Справа панель,
в которой открыта **ровно одна форма**: либо группы, либо выбранного потока. Обе
панели скроллятся внутри себя, страница не двигается — выбранная форма всегда
открывается на одном и том же месте.

Клик по группе открывает форму группы (настройки, антифлуд, обзорная таблица её
потоков и ссылки, которые отдаёт движок). Клик по потоку — в дереве или через
«edit →» в этой таблице — заменяет содержимое панели формой потока; ссылка
назад в её шапке возвращает к группе.

Форма потока (вкладки): Главное · Устройства · WAP · Гео ·
Фильтры · UA/ОС/Бренд · Расписание · Лимит · Боты · Remote · API.

Правки живут в браузере до кнопки **Save all** (работает и `Ctrl`/`Cmd`+`S`).
Рядом с кнопкой загорается метка о несохранённых изменениях, а уход из раздела
или закрытие вкладки сначала спросят подтверждение. Пустые и дублирующиеся ID
групп отлавливаются до отправки запроса.
