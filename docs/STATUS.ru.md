...
| cmd/engine | 66.1% | httptest-конвейер + хелперы + **e2e_test.go** (23 сквозных сценария: все типы редиректа, все макросы, боты, гео, фильтры, операторы, распределение, лимиты, фаервол, separation, расписание, chance, api-режим, матрица трафика) |
...
тесты под `integration` скипаются (без них 29.7%). Значение 77.0% — последнее
