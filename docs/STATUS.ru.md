...
- **Проход по надёжности (2026-09-07, PR #13–#27).** Сгруппировано по тому, что
  каждое защищает:
  ...
  *Админка.* См. `SECURITY.md` §3: оракул логина на мёртвом Redis, ограничение
  стоимости хеширования, логин вон из логов (#23, доехало через #25), один
  писатель и атомарная запись для хэша пароля (#22) и лимитер логина, который
  мог забанить администратора навсегда (#21).
...
- **Замечание о процессе.** #15–#17 были стеком: каждый целился в предыдущую
  *ветку*, а не в `main`. #13 слили первым, цепочка оборвалась, и GitHub
  ...
  Работа по транспорту и seplist пролежала вне `main` десять дней, пока её не
...
тесты под `integration` скипаются (без них 27.5%). Значение 77.0% — последнее
