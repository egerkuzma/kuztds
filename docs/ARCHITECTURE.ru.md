...
2. **Горячий путь без лишнего блокирующего I/O**: логи async; счётчики Redis;
   ...
   Исключение — четыре опциональные фичи, которые *действительно* ходят в
   файловую систему на каждом запросе, где включены: `separation` перечитывает и
   сканирует свой `.dat` (`separationOut`), `save_keys`/`save_keys_se` дописывают
   ...
   размен пропускной способности на фичу; всё остальное остаётся в памяти.
